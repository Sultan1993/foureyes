...
RID="pr-N"
```
Then put the run dir INSIDE the git directory, never in the worktree:
```bash
RUN="$(git rev-parse --git-dir)/foureyes-review/$RID"; mkdir -p "$RUN"
```
This plugin ships no `.gitignore` rule into consumer repos, so a worktree path
leaves a stray untracked directory in every repo you review in — and a reviewer
run should not show up in the diff of the thing being reviewed. Inside the git
dir it is untracked by construction. (`foureyes-build` does the same, for the
same reason.)
...
- Local: newest of `docs/foureyes/specs/*.md` or `docs/foureyes/plans/*.md`. Read it (cap ~4KB).
...
- `raisedBy == ["codex"]` → Claude refutes: `Agent` tool, `subagent_type: foureyes-refute-critic`, `prompt` = REFUTE_INPUT, and `model: <--claude-model or opus>` — always explicit.

Spec: docs/superpowers/specs/2026-07-22-foureyes-three-commands-design.md

# foureyes 1.6.0 — three-command restructure — implementation plan

**Goal:** Implement the approved spec: `foureyes-brainstorm` (new),
`foureyes-build` (plan input + verify/re-bless), `foureyes-review` (Opus
...
**Goal:** `agents/foureyes-drafter.md` — Fable drafter subagent.
**Files:** Create `agents/foureyes-drafter.md`
**Steps:** Frontmatter: `name: foureyes-drafter`, `description` (drafts specs
and parallel-ready plans for foureyes-brainstorm; returns CONTENT, never
...
**Verify:** `grep -q 'model: fable' agents/foureyes-drafter.md && ! grep -qE '^tools:.*(Write|Edit|Bash)' agents/foureyes-drafter.md && grep -q 'mcp__context7__resolve-library-id' agents/foureyes-drafter.md && grep -q 'Spec:' agents/foureyes-drafter.md`

```json:metadata
{"files": ["agents/foureyes-drafter.md"], "modelTier": "mechanical", "verifyCommand": "grep checks per Verify", "acceptanceCriteria": ["model: fable", "no write tools", "exact mcp tool names", "content-return + format contract in body"]}
```
...
**Files:** Modify `scripts/codex-critic.sh`; create
`tests/codex-critic.test.sh`, `tests/stubs/codex`
...
**Acceptance Criteria:**
- cases 1–17 of the spec's matrix pass via `bash tests/codex-critic.test.sh`
- review mode stdout unchanged; seam modes ignore `CODEX_CRITIC_MODEL`
**Verify:** `bash tests/codex-critic.test.sh`

```json:metadata
{"files": ["scripts/codex-critic.sh", "tests/codex-critic.test.sh", "tests/stubs/codex"], "modelTier": "standard", "verifyCommand": "bash tests/codex-critic.test.sh", "acceptanceCriteria": ["matrix cases 1-17 + 8d byte-identity pass", "self-invalidation", "strict pass-token clean", "sidecar-then-marker order", "verify + --spec-only", "model pin for seam modes"]}
```

### Task 3: gate hook — hash validation
**Goal:** `foureyes-build-gate.sh` validates hash-bearing plan markers.
**Files:** Modify `foureyes/hooks/foureyes-build-gate.sh`; create
`tests/build-gate.test.sh`
...
**Verify:** `bash tests/build-gate.test.sh`

```json:metadata
{"files": ["foureyes/hooks/foureyes-build-gate.sh", "tests/build-gate.test.sh"], "modelTier": "standard", "verifyCommand": "bash tests/build-gate.test.sh", "acceptanceCriteria": ["hash recompute + deny on mismatch incl. spec", "invocation-path binding with cardinality exactly one", "empty marker legacy allow", "malformed marker deny", "cases 18-22 incl. duplicate-token denials pass"]}
```

### Task 4: plan-viz — generator + tests
**Goal:** Pure-node plan visualizer with deterministic detectors.
**Files:** Create `skills/foureyes-brainstorm/lib/plan-viz.mjs`,
`skills/foureyes-brainstorm/lib/plan-viz.test.mjs`
...
**Verify:** `node --test skills/foureyes-brainstorm/lib/plan-viz.test.mjs`

```json:metadata
{"files": ["skills/foureyes-brainstorm/lib/plan-viz.mjs", "skills/foureyes-brainstorm/lib/plan-viz.test.mjs"], "modelTier": "standard", "verifyCommand": "node --test skills/foureyes-brainstorm/lib/plan-viz.test.mjs", "acceptanceCriteria": ["all detectors per pinned semantics", "Kahn waves + cycle handling", "--json test surface", "self-contained dual-theme HTML"]}
```

### Task 5: foureyes-review — Opus default
**Goal:** Flip the Claude side to Opus at every layer that matters.
**Files:** Modify `agents/foureyes-review-critic.md`,
`agents/foureyes-refute-critic.md`,
`skills/foureyes-review/SKILL.md`,
`skills/foureyes-review/lib/review-synth.mjs`
...
**Verify:** `bash -c 'set -e; for f in agents/foureyes-review-critic.md agents/foureyes-refute-critic.md; do grep -q "^model: opus" "$f"; done; grep -q "claudeModel:<--claude-model or \"opus\">" skills/foureyes-review/SKILL.md; grep -qc "model: <--claude-model or opus>" skills/foureyes-review/SKILL.md; grep -q "meta.claudeModel || .opus." skills/foureyes-review/lib/review-synth.mjs; node --test skills/foureyes-review/lib/review-synth.test.mjs >/dev/null'`

```json:metadata
{"files": ["agents/foureyes-review-critic.md", "agents/foureyes-refute-critic.md", "skills/foureyes-review/SKILL.md", "skills/foureyes-review/lib/review-synth.mjs"], "modelTier": "mechanical", "verifyCommand": "greps + node --test review-synth.test.mjs", "acceptanceCriteria": ["frontmatters opus", "explicit model passing", "fallback opus", "existing tests green"]}
```

### Task 6: plan-critic — tier-assignment audit
**Goal:** Add the tier audit dimension to the plan critic prompt.
**Files:** Modify `agents/foureyes-plan-critic.md`
...
**Verify:** `grep -qi 'tier' agents/foureyes-plan-critic.md && grep -qi 'spec completeness' agents/foureyes-plan-critic.md`

```json:metadata
{"files": ["agents/foureyes-plan-critic.md"], "modelTier": "mechanical", "verifyCommand": "grep tier + spec completeness", "acceptanceCriteria": ["audit dimension with both directions and tie-break"]}
```

### Task 7: foureyes-brainstorm SKILL
**Goal:** Coordinator instructions for the new command.
**Files:** Create `skills/foureyes-brainstorm/SKILL.md`
...
**Verify:** `bash -c 'set -e; f=skills/foureyes-brainstorm/SKILL.md; for a in foureyes-drafter CLAUDE_CODE_SUBAGENT_MODEL MARKER_DIR SPEC_PATH PLAN_PATH plan-viz .done -- --continue --skip-critics Spec: CODEX_CRITIC_EFFORT; do [ "$a" = -- ] && continue; grep -q -- "$a" "$f"; done; ! grep -qE "TaskCreate:|invoke TaskCreate|call TaskCreate" "$f"; ! grep -q "AskUserQuestion" "$f" || grep -q "never.*AskUserQuestion\|Never ask" "$f"'`

```json:metadata
{"files": ["skills/foureyes-brainstorm/SKILL.md"], "modelTier": "standard", "verifyCommand": "wiring greps + zero TaskCreate", "acceptanceCriteria": ["preflight", "seam loops via MARKER_DIR", "hard stop + .done", "--continue + --skip-critics", "no TaskCreate/AskUserQuestion"]}
```

### Task 8: foureyes-build SKILL — plan input, verify, re-bless
**Goal:** Rewrite build around the trust ladder.
**Files:** Modify `skills/foureyes-build/SKILL.md`
...
plan: invoke Skill `foureyes:foureyes-brainstorm` with `--continue`
...
```bash
( grep -q 'model: fable' agents/foureyes-drafter.md && ! grep -qE '^tools:.*(Write|Edit|Bash)' agents/foureyes-drafter.md && grep -q 'mcp__context7__resolve-library-id' agents/foureyes-drafter.md && grep -q 'Spec:' agents/foureyes-drafter.md ) && \
( bash tests/codex-critic.test.sh ) && \
( bash tests/build-gate.test.sh ) && \
( node --test skills/foureyes-brainstorm/lib/plan-viz.test.mjs ) && \
( bash -c 'set -e; for f in agents/foureyes-review-critic.md agents/foureyes-refute-critic.md; do grep -q "^model: opus" "$f"; done; grep -q "claudeModel:<--claude-model or \"opus\">" skills/foureyes-review/SKILL.md; grep -qc "model: <--claude-model or opus>" skills/foureyes-review/SKILL.md; grep -q "meta.claudeModel || .opus." skills/foureyes-review/lib/review-synth.mjs; node --test skills/foureyes-review/lib/review-synth.test.mjs >/dev/null' ) && \
( grep -qi 'tier' agents/foureyes-plan-critic.md && grep -qi 'spec completeness' agents/foureyes-plan-critic.md ) && \
( bash -c 'set -e; f=skills/foureyes-brainstorm/SKILL.md; for a in foureyes-drafter CLAUDE_CODE_SUBAGENT_MODEL MARKER_DIR SPEC_PATH PLAN_PATH plan-viz .done -- --continue --skip-critics Spec: CODEX_CRITIC_EFFORT; do [ "$a" = -- ] && continue; grep -q -- "$a" "$f"; done; ! grep -qE "TaskCreate:|invoke TaskCreate|call TaskCreate" "$f"; ! grep -q "AskUserQuestion" "$f" || grep -q "never.*AskUserQuestion\|Never ask" "$f"' ) && \
( bash -c 'set -e; f=skills/foureyes-build/SKILL.md; for a in verify spec-only planPath= Spec: foureyes-brainstorm slug CODEX_CRITIC_EFFORT; do grep -q -- "$a" "$f"; done; grep -q -- "--continue --skip-critics" "$f"; grep -qE "create NO run dir|NO run dir created|no run dir" "$f"; grep -qiE "skip Seam 3|Seam 3 is also skipped|skip .*code.critic" "$f"; grep -qiE "nothing is approved|cannot fake|never forges" "$f"; ! grep -q OVERRIDE "$f"; ! grep -q "seam-gate tasks" "$f"; ! grep -qE "TaskCreate:|invoke TaskCreate|call TaskCreate" "$f"' ) && \
( bash tests/codex-critic.test.sh >/dev/null ) && \
( bash tests/build-gate.test.sh >/dev/null ) && \
( node --test skills/foureyes-brainstorm/lib/plan-viz.test.mjs skills/foureyes-review/lib/review-synth.test.mjs >/dev/null ) && \
( grep -q '"version": "1.6.0"' foureyes/.claude-plugin/plugin.json ) && \
( grep -q foureyes-brainstorm README.md ) && echo BARRIER-GREEN
```

```json:metadata
{"files": ["foureyes/.claude-plugin/plugin.json", "README.md"], "modelTier": "standard", "verifyCommand": "the literal ordered barrier command in this task's Verify block (Tasks 1-8 verifies chained, then the four suites, then version/README greps; prints BARRIER-GREEN)", "acceptanceCriteria": ["1.6.0", "README three commands", "all suites green", "Tasks 1-8 Verify commands re-executed at the barrier"], "blockedBy": [1, 2, 3, 4, 5, 6, 7, 8]}
```

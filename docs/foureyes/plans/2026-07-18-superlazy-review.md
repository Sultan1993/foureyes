# foureyes-review Implementation Plan
...
- Architecture: coordinator-driven (B), matching foureyes-build; Workflow tool avoided (no shell in its sandbox).
...
_All paths in this section and in each task's **Files**/**Create** lines are relative to the plugin dir `foureyes/` (e.g. `agents/…` → `agents/…`). Shell commands that `cd` to the repo root (`/Users/sultan/Development/lazypowers`) use the full `foureyes/…` prefix._

- `agents/foureyes-review-critic.md` — shared reviewer prompt (Claude subagent type AND read by `codex-critic.sh review`). Emits findings JSON.
- `agents/foureyes-refute-critic.md` — shared refuter prompt (`codex-critic.sh refute` + Claude subagent type). Emits `{refuted, reason}`.
- `skills/foureyes-review/lib/review-synth.mjs` — pure deterministic logic + CLI (`bucket`, `render`). All non-model logic lives here.
- `skills/foureyes-review/lib/review-synth.test.mjs` — `node --test` unit tests.
- `skills/foureyes-review/SKILL.md` — coordinator instructions (the pipeline).
- `skills/foureyes-review/test/` — golden-diff fixture + acceptance doc.
- `.claude-plugin/plugin.json` — version bump.

`scripts/codex-critic.sh` is reused UNCHANGED: it resolves `agents/foureyes-${critic}-critic.md`, so `codex-critic.sh review` → `foureyes-review-critic.md` and `codex-critic.sh refute` → `foureyes-refute-critic.md`.
...
**Files:**
- Create: `agents/foureyes-review-critic.md`
- Create: `agents/foureyes-refute-critic.md`
...
**Verify:** `cd /Users/sultan/Development/lazypowers && printf 'WORKTREE: %s\nBASE_SHA: HEAD~1\nHEAD_SHA: HEAD\nDIMENSIONS: correctness\nCONTEXT:\n(none)\n' "$(pwd)" | ./scripts/codex-critic.sh review | node -e 'const s=require("fs").readFileSync(0,"utf8"); const o=JSON.parse(s); if(!("findings" in o)) throw new Error("no findings key"); console.log("ok",o.verdict)'` → prints `ok <verdict>`

**Steps:**

- [ ] **Step 1: Write `agents/foureyes-review-critic.md`**

```markdown
---
name: foureyes-review-critic
description: >
  Cross-model code reviewer for foureyes-review. Reviews a diff across six
...
- [ ] **Step 2: Write `agents/foureyes-refute-critic.md`**

```markdown
---
name: foureyes-refute-critic
description: >
  Refutation reviewer for foureyes-review. Given ONE finding and the diff, tries
  to refute it and emits {refuted, reason} JSON. Read-only.
tools: Read, Grep, Glob, Bash, WebSearch, mcp__context7__resolve-library-id, mcp__context7__query-docs
model: opus
---

# foureyes-review refuter
...
printf 'WORKTREE: %s\nBASE_SHA: HEAD~1\nHEAD_SHA: HEAD\nDIMENSIONS: correctness\nCONTEXT:\n(none)\n' "$(pwd)" \
  | ./scripts/codex-critic.sh review 2>/dev/null \
...
```bash
cd /Users/sultan/Development/lazypowers
git add agents/foureyes-review-critic.md agents/foureyes-refute-critic.md
git commit -m "foureyes-review: reviewer + refuter prompts (JSON output)"
```

```json:metadata
{"files": ["agents/foureyes-review-critic.md", "agents/foureyes-refute-critic.md"], "verifyCommand": "cd /Users/sultan/Development/lazypowers && printf 'WORKTREE: %s\\nBASE_SHA: HEAD~1\\nHEAD_SHA: HEAD\\nDIMENSIONS: correctness\\nCONTEXT:\\n(none)\\n' \"$(pwd)\" | ./scripts/codex-critic.sh review | node -e 'JSON.parse(require(\"fs\").readFileSync(0,\"utf8\"))'", "acceptanceCriteria": ["codex-critic.sh review returns parseable {verdict,summary,findings}", "codex-critic.sh refute returns {refuted,reason}", "frontmatter registers both as subagent types"], "modelTier": "standard"}
```
...
**Files:**
- Create: `skills/foureyes-review/lib/review-synth.mjs`
- Create: `skills/foureyes-review/lib/review-synth.test.mjs`
...
**Verify:** `node --test /Users/sultan/Development/lazypowers/skills/foureyes-review/lib/review-synth.test.mjs` → all tests pass
...
Run: `node --test /Users/sultan/Development/lazypowers/skills/foureyes-review/lib/review-synth.test.mjs`
Expected: FAIL — `Cannot find module './review-synth.mjs'`.

- [ ] **Step 3: Write `skills/foureyes-review/lib/review-synth.mjs`**

```javascript
#!/usr/bin/env node
// review-synth.mjs — pure synthesis for foureyes-review. No model calls, no network.
...
export function renderReport(meta, ranked) {
  ...
  const out = [];
  out.push(`# foureyes-review — ${meta.target || 'diff'}`, '');
...
export function renderDigest(meta, ranked) {
  ...
  return [`foureyes-review: ${c.Critical} Critical, ${c.Important} Important, ${c.Minor} Minor`, ...top].join('\n');
}
...
```bash
cd /Users/sultan/Development/lazypowers
git add skills/foureyes-review/lib/review-synth.mjs skills/foureyes-review/lib/review-synth.test.mjs
git commit -m "foureyes-review: synthesis lib (bucket/rank/refute/render) + tests"
```

```json:metadata
{"files": ["skills/foureyes-review/lib/review-synth.mjs", "skills/foureyes-review/lib/review-synth.test.mjs"], "verifyCommand": "node --test /Users/sultan/Development/lazypowers/skills/foureyes-review/lib/review-synth.test.mjs", "acceptanceCriteria": ["bucketFindings agreement/single + stable ids", "rankFindings severity then agreement then dimension", "applyRefutations drops refuted singles keeps agreed", "renderReport markdown shape", "node --test passes"], "modelTier": "standard"}
```
...
**Files:**
- Create: `skills/foureyes-review/SKILL.md`

**Acceptance Criteria:**
- [ ] Skill parses no-arg (local), numeric-arg (PR), and `--base/--post/--serial/--dimensions` flags.
- [ ] Resolves plugin paths via the cache glob; runs Claude via `Agent subagent_type foureyes-review-critic` and Codex via `codex-critic.sh review`, in parallel.
...
- [ ] References only files/agents that exist (foureyes-review-critic, foureyes-refute-critic, review-synth.mjs, codex-critic.sh).

**Verify:** `for a in 'subagent_type .foureyes-review-critic' 'codex-critic.sh review' 'review-synth.mjs bucket' 'foureyes-refute-critic' '--post' 'worktree remove'; do grep -q "$a" /Users/sultan/Development/lazypowers/skills/foureyes-review/SKILL.md || echo "MISSING: $a"; done` → prints nothing

**Steps:**

- [ ] **Step 1: Write `skills/foureyes-review/SKILL.md`**
...
# foureyes-review — cross-model code review
...
- Numeric arg (`foureyes-review 142`) → review GitHub PR #142.
...
RID="pr-N"
```
Then: `RUN=".foureyes-review/$RID"; mkdir -p "$RUN"`.
...
- Local: newest of `docs/superpowers/specs/*.md`, `docs/superpowers/plans/*.md`, or a `.foureyes-build/*/` plan. Read it (cap ~4KB).
...
- Claude: `Agent` tool, `subagent_type: foureyes-review-critic`, `prompt` = the INPUT block. (Its system prompt is the reviewer; the INPUT is the assignment.)
...
- `raisedBy == ["codex"]` → Claude refutes: `Agent` tool, `subagent_type: foureyes-refute-critic`, `prompt` = REFUTE_INPUT.
...
Run:
```bash
for a in 'subagent_type: foureyes-review-critic' 'codex-critic.sh' 'review-synth.mjs bucket' 'foureyes-refute-critic' '--post' 'worktree remove'; do
  grep -q "$a" /Users/sultan/Development/lazypowers/skills/foureyes-review/SKILL.md || echo "MISSING: $a"
done
test -f /Users/sultan/Development/lazypowers/agents/foureyes-review-critic.md || echo "MISSING review-critic"
test -f /Users/sultan/Development/lazypowers/agents/foureyes-refute-critic.md || echo "MISSING refute-critic"
test -f /Users/sultan/Development/lazypowers/skills/foureyes-review/lib/review-synth.mjs || echo "MISSING synth"
```
Expected: no output.

Note: the SKILL.md lives at `skills/foureyes-review/SKILL.md`; adjust the create path to inside `foureyes/` to match the plugin root. (All plugin content is under `foureyes/`.)

- [ ] **Step 3: Commit**

```bash
cd /Users/sultan/Development/lazypowers
git add skills/foureyes-review/SKILL.md
git commit -m "foureyes-review: coordinator SKILL.md pipeline"
```

```json:metadata
{"files": ["skills/foureyes-review/SKILL.md"], "verifyCommand": "for a in 'subagent_type: foureyes-review-critic' 'codex-critic.sh' 'review-synth.mjs bucket' 'foureyes-refute-critic' '--post' 'worktree remove'; do grep -q \"$a\" /Users/sultan/Development/lazypowers/skills/foureyes-review/SKILL.md || echo MISSING $a; done", "acceptanceCriteria": ["parses local/PR/flags", "parallel Claude+Codex reviews", "bucket->refute-singles->render", "codex-down + empty-diff + no-base handling", "--post confirm + worktree cleanup", "references only existing files/agents"], "modelTier": "standard"}
```

Note on paths: the lib and SKILL live UNDER the plugin dir `foureyes/`, i.e. `skills/foureyes-review/…` and `skills/foureyes-review/lib/…`. Task 2's create paths must be prefixed with `foureyes/` too. (The plan shows repo-root-relative paths in a couple of Verify commands for brevity; the canonical location is under `foureyes/`.)
...
**Files:**
- Modify: `foureyes/.claude-plugin/plugin.json`

**Acceptance Criteria:**
- [ ] `version` bumped to `1.4.0`.
- [ ] description mentions the `foureyes-review` two-model review skill.
- [ ] File is valid JSON.

**Verify:** `node -e 'const p=require("/Users/sultan/Development/lazypowers/foureyes/.claude-plugin/plugin.json"); if(p.version!=="1.4.0") throw new Error("version"); if(!/foureyes-review/.test(p.description)) throw new Error("desc"); console.log("ok")'` → prints `ok`

**Steps:**

- [ ] **Step 1: Edit `foureyes/.claude-plugin/plugin.json`**

Set `"version": "1.4.0"` and append to the description: ` Also ships foureyes-review: a two-model (Claude + Codex) adversarial code-review skill.`

- [ ] **Step 2: Verify + commit**

```bash
cd /Users/sultan/Development/lazypowers
node -e 'const p=require("./foureyes/.claude-plugin/plugin.json"); if(p.version!=="1.4.0"||!/foureyes-review/.test(p.description)) throw new Error("bad"); console.log("ok")'
git add foureyes/.claude-plugin/plugin.json
git commit -m "foureyes: v1.4.0 — add foureyes-review skill"
```

```json:metadata
{"files": ["foureyes/.claude-plugin/plugin.json"], "verifyCommand": "node -e 'const p=require(\"/Users/sultan/Development/lazypowers/foureyes/.claude-plugin/plugin.json\"); if(p.version!==\"1.4.0\") throw new Error(\"v\"); console.log(\"ok\")'", "acceptanceCriteria": ["version 1.4.0", "description mentions foureyes-review", "valid JSON"], "modelTier": "mechanical"}
```
...
**Files:**
- Create: `skills/foureyes-review/test/fixture.diff`
- Create: `skills/foureyes-review/test/acceptance.md`
- Modify: `README.md` (repo root) — add a foureyes-review section.
...
- [ ] `acceptance.md` documents: apply the fixture to a scratch repo, run `foureyes-review`, and the EXPECTED qualitative result — the real Critical survives and is agreement- or single-but-unrefuted, the false one is refuted out.
...
- [ ] README documents install + usage (`foureyes-review`, `foureyes-review <PR#>`, flags).

**Verify:** `node --test /Users/sultan/Development/lazypowers/skills/foureyes-review/lib/review-synth.test.mjs && test -f /Users/sultan/Development/lazypowers/skills/foureyes-review/test/acceptance.md && grep -q foureyes-review /Users/sultan/Development/lazypowers/README.md && echo ok` → prints `ok`
...
- [ ] **Step 3: Add a `foureyes-review` section to `README.md`** (install from the fork, the two invocation modes, the flags, and the "advisory, two-model" positioning).

- [ ] **Step 4: Verify + commit**

```bash
cd /Users/sultan/Development/lazypowers
node --test skills/foureyes-review/lib/review-synth.test.mjs
test -f skills/foureyes-review/test/acceptance.md && grep -q foureyes-review README.md && echo ok
git add skills/foureyes-review/test README.md
git commit -m "foureyes-review: golden fixture, acceptance doc, README"
```

```json:metadata
{"files": ["skills/foureyes-review/test/fixture.diff", "skills/foureyes-review/test/acceptance.md", "README.md"], "verifyCommand": "node --test /Users/sultan/Development/lazypowers/skills/foureyes-review/lib/review-synth.test.mjs && test -f /Users/sultan/Development/lazypowers/skills/foureyes-review/test/acceptance.md && grep -q foureyes-review /Users/sultan/Development/lazypowers/README.md && echo ok", "acceptanceCriteria": ["fixture has planted Critical + plausible-false", "acceptance.md documents live run + expected outcome", "node --test referenced as automated gate", "README documents install/usage"], "modelTier": "standard"}
```
...
All plugin content lives UNDER `foureyes/`. Canonical create paths:
- `agents/foureyes-review-critic.md`, `agents/foureyes-refute-critic.md`
- `skills/foureyes-review/SKILL.md`
- `skills/foureyes-review/lib/review-synth.mjs` (+ `.test.mjs`)
- `skills/foureyes-review/test/…`
- `foureyes/.claude-plugin/plugin.json`

A few Verify commands above use repo-root-relative shorthands; the canonical location is under `foureyes/`.

## Post-implementation

Install the new version and smoke-test on this very repo:
```bash
claude plugin update foureyes          # or marketplace update + reload
# in a repo with a branch: foureyes-review --base main
```

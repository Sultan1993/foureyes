...
- Every remaining "plan" refers to nothing; `docs/foureyes/plans` absent. The word "Steps" appears only in Step 0.5's explanation of the `has-steps` stop; the E2 assignment line (`Goal / Files / Acceptance Criteria / Verify verbatim`) does not list it, so the phrase `Verify / Steps` is absent.

**Verify:** `f=skills/foureyes-build/SKILL.md; grep -q 'Has Astra ever read this spec' $f && grep -q "'^## spec '" $f && grep -q 'Cross-Task Contracts' $f && grep -q 'has-steps' $f && ! grep -q 'PLAN_DOC\|foureyes/plans\|Verify / Steps' $f && grep -q 'SPEC_DOC' $f && echo OK`

```json:metadata
{"files": ["skills/foureyes-build/SKILL.md"], "modelTier": "standard", "verifyCommand": "f=skills/foureyes-build/SKILL.md; grep -q 'Has Astra ever read this spec' $f && grep -q \"'^## spec '\" $f && grep -q 'Cross-Task Contracts' $f && grep -q 'has-steps' $f && ! grep -q 'PLAN_DOC\\|foureyes/plans\\|Verify / Steps' $f && grep -q 'SPEC_DOC' $f && echo OK", "acceptanceCriteria": ["argument and classifier are spec-based; a legacy plan stops at Step 0.5 on has-steps with a one-line explanation and dispatches nothing", "E2 and E5 forward contracts and constraints; the E2 assignment line does not list Steps", "S3 and the critique log use the spec only"]}
```
...
**Verify:** `t=tests/prose-contracts.test.sh; bash -n $t && bash -n tests/codex-critic.test.sh && grep -q 'Has Astra ever read this spec' $t && grep -qF '/^## Step 5/' $t && ! grep -qF '/^## Step 7/' $t && ! grep -q 'DRAFT: part.*REMAINING\|22h\|25a' $t && grep -q 'plan-critic.md' $t && grep -q 'spliceTasks' $t && grep -q 'CLAUDE_CODE_MAX_OUTPUT_TOKENS' $t && grep -q 'Cross-Task Contracts' $t && grep -q 'for m in spec code' tests/codex-critic.test.sh && echo OK`

The negative assertions (c), (g), (h) necessarily mention `TASKS-PER-PART`, `spliceTasks` and `plan-critic` as grep operands; the Verify checks that those assertions exist, not that the words are absent from the test source.

```json:metadata
{"files": ["tests/prose-contracts.test.sh", "tests/codex-critic.test.sh"], "modelTier": "standard", "verifyCommand": "t=tests/prose-contracts.test.sh; bash -n $t && bash -n tests/codex-critic.test.sh && grep -q 'Has Astra ever read this spec' $t && grep -qF '/^## Step 5/' $t && ! grep -qF '/^## Step 7/' $t && ! grep -q 'DRAFT: part.*REMAINING\\|22h\\|25a' $t && grep -q 'plan-critic.md' $t && grep -q 'spliceTasks' $t && grep -q 'CLAUDE_CODE_MAX_OUTPUT_TOKENS' $t && grep -q 'Cross-Task Contracts' $t && grep -q 'for m in spec code' tests/codex-critic.test.sh && echo OK", "acceptanceCriteria": ["plan-encoding assertions removed or rewritten as listed, s7 helper moved to Step 5", "eight new contract assertions added, including the negative ones that name their operands", "codex-critic budget loop is spec code", "verify runs independently; the full suite is the coordinator's barrier"]}
```

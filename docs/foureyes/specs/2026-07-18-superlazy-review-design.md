# foureyes-review — design spec

Date: 2026-07-18
Status: approved (design), pending implementation plan
Repo: `Sultan1993/forge` (plugin `foureyes`)
...
- `foureyes-review` — review the current branch vs its base. Base auto-detected
  as `git merge-base <default-branch> HEAD`; override with `--base <ref>`.
- `foureyes-review <PR#>` — `gh pr checkout` the PR into a **dedicated git
...
- A plan/spec from a foureyes-build run dir or `docs/superpowers/specs/`.
...
The `foureyes-review` skill instructs the coordinator (Claude Code, this
...
2. **Review (two parallel jobs)** — each model reviews the whole diff across all
   ...
   - Claude side: the `Agent` tool with the `foureyes-review-critic` prompt,
...
4. **Cross-refute (parallel, SINGLE only)** — hand each single-model finding to
   the *other* model using the `foureyes-refute-critic` prompt (Codex via
...
6. **Deliver** — write `.foureyes-review/<run-id>/report.md` + a terminal digest
...
- `skills/foureyes-review/SKILL.md` — the coordinator instructions. Parses
...
- `skills/foureyes-review/lib/review-synth.mjs` — pure, unit-tested node module
...
- `agents/foureyes-review-critic.md` — the shared **plan-agnostic** review
  ...
  findings JSON above.
- `agents/foureyes-refute-critic.md` — the refutation prompt: given one finding
...
- `scripts/codex-critic.sh` — **reused unchanged**: it already resolves
  `agents/foureyes-${critic}-critic.md`, so `codex-critic.sh review` reads
  `foureyes-review-critic.md`. Same model (gpt-5.6-sol) / effort (high) /
...
Complements `/code-review` (single-model, fast, current diff). `foureyes-review`

# foureyes — three-command restructure — design spec
...
Repo: `Sultan1993/forge` (plugin `foureyes`), version `1.5.0` → `1.6.0`

## Purpose

Restructure `foureyes` into three standalone commands with a clean division of
...
### 1. `foureyes-brainstorm` (NEW, standalone)
...
2. Coordinator initializes the run dir `.foureyes-build/<run-id>/` (same
...
3. **Fable drafter** (`foureyes-drafter` agent, `model: fable`) returns the design
...
8. Ends by **printing** the paths (spec, plan, tasks, sidecar, HTML) and stops.
   ...
   .foureyes-build/<run-id>/.done`. The run dir's job is done — approval
...
`foureyes-build` invokes brainstorm for its no-plan path.

### 2. `foureyes-build [plan]` (MODIFY existing)
...
- **Without a plan** (no path, or path without `.tasks.json`): invoke
  `foureyes-brainstorm --continue`, then execute without stopping.
...
- Step 1's invocation of `superpowers-extended-cc:brainstorming` is replaced by
  `foureyes-brainstorm --continue`, deleting the OVERRIDE prose at current
...
- **`--skip-critics` — the one loud bypass, fully specified.** Announce
  ...
  `foureyes-brainstorm --continue --skip-critics`, which still gathers
...
### 3. `foureyes-review` (MODIFY existing)
...
report:
- `agents/foureyes-review-critic.md` frontmatter: `model: sonnet` → `model: opus`
- `agents/foureyes-refute-critic.md` frontmatter: `model: sonnet` → `model: opus`
- `skills/foureyes-review/SKILL.md`: always pass `model: <--claude-model or opus>`
...
- Seam modes **capture** Codex output, print it, then parse it: VERDICT =
  ...
  foureyes-review): seam-mode capture may normalize trailing newlines, which
...
| Flow | Run dir | At end |
|---|---|---|
| `foureyes-brainstorm` (standalone) | creates `<slug>` (mutex-checked as today) | **`.done` at the hard stop** — its job ends with the sidecar written; leaving it active would false-BUSY a later session's build |
| `foureyes-brainstorm --continue` | creates `<slug>` | left **active**; build keeps using it in-session (markers present, gate armed), `.done` at build's finish as today |
| `foureyes-build <plan>` | mutex scans the **whole slug family** (`<plan-slug>` and every `<plan-slug>-N`): any family member ACTIVE and owned by a foreign session → **BUSY, stop** (a `.done` base must not let allocation sidestep a foreign `-2`); a family member active and owned by THIS session → rebind and reuse it. Only then allocate the first **genuinely absent** name in the family. `.done` dirs are never reused, cleared, or rebound — historical records the gate ignores | `verify`/re-bless mint markers into it; `.done` at finish |
...
**New**
- `skills/foureyes-brainstorm/SKILL.md`
- `skills/foureyes-brainstorm/lib/plan-viz.mjs` + `plan-viz.test.mjs`
- `agents/foureyes-drafter.md` — `model: fable`, tools: `Read`,
...
**Modified**
- `scripts/codex-critic.sh` — capture (drop `exec`), strict pass-token
...
- `foureyes/hooks/foureyes-build-gate.sh` — validate hash-bearing plan
...
- `skills/foureyes-build/SKILL.md` — plan resolution, verify/re-bless
  paths, brainstorm delegation, OVERRIDE prose removal
- `skills/foureyes-review/SKILL.md` — explicit model passing, opus default
- `agents/foureyes-review-critic.md` — `model: opus`
- `agents/foureyes-refute-critic.md` — `model: opus`
- `agents/foureyes-plan-critic.md` — tier-assignment audit dimension
- `skills/foureyes-review/lib/review-synth.mjs` — `'sonnet'` → `'opus'`
- `foureyes/.claude-plugin/plugin.json` — `1.6.0` + description
...
`foureyes-build-gate.sh` (direct invocation with crafted stdin, like the
existing hook tests):

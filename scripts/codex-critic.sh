#!/usr/bin/env bash
# codex-critic.sh — run a foureyes critic on OpenAI Codex instead of a Claude subagent.
#
# Usage:  codex-critic.sh <approach|spec|plan|code|review|refute>   (crafted inputs piped on stdin)
#
# `approach` is a PROPOSER, not a critic: it gets only the brief + repo and
# returns candidate approaches, concurrently with a Claude drafter that cannot
# see it. Non-seam, so no round budget and no GATE line — the coordinator merges
# both sets into one menu. (Naming: the prompt path template below forces
# foureyes-approach-critic.md even though the role is proposer.)
#
# The critic's instruction body (including the exact VERDICT block spec) is read
# from ../agents/foureyes-<critic>-critic.md with its YAML frontmatter stripped,
...
critic="${1:?usage: codex-critic.sh <approach|spec|plan|code|review|refute>}"
...
prompt_file="$here/../agents/foureyes-${critic}-critic.md"
...
# (command substitution would strip trailing newlines, and foureyes-review
...
# Time the call HERE, not at the call sites: a seam's value is only a decision
# once you know its price, and a coordinator asked to note the clock will forget.
# Only seam modes are timed — review/refute `exec` above, replacing this shell,
...
_secs=$(( $(date +%s) - _t0 ))
printf '%s\n' "$out"
# Deliberately avoids the word "round": stderr is grepped, and a second line
# saying "round N" would collide with the gate's own messages.
echo "codex-critic: $critic r$ROUND took ${_secs}s" >&2
